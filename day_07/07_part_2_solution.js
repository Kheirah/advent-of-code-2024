...
for (const calibration of calibrationValues) {
  ...
  let permutations = Math.pow(3, values.length);

  while (permutations) {
    const binary = permutations.toString(3).padStart(values.length, "0");
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (binary[i - 1] === "0") {
        ...
        currentSum = Number(currentSum + "" + value);
      }
    }
    if (currentSum == sum) {
